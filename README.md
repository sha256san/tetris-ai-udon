...
### 3. 4体同時 並列進化学習バッチの起動 & 停止 (SSH切断保護対応)

#### ① SSH切断完全耐性 デーモンモード（バックグラウンド永続実行）
```bash
# 【書式】 ./start_training_daemon.sh <トレーニング回数/ラウンド> <繰り返す回数(0=無制限)>

# 例1: 100回ずつ 5ラウンド繰り返して自動終了（計2,000ゲーム探索後、プロセス完全終了・メモリ全解放）
./start_training_daemon.sh 100 5

# 例2: 500回ずつ 10ラウンド繰り返して自動終了（計20,000ゲーム探索後、プロセス完全終了・メモリ全解放）
./start_training_daemon.sh 500 10

# 例3: 100回ずつ 無制限にエンドレス反復
./start_training_daemon.sh 100 0

# 【リアルタイムログ確認】(確認を閉じても裏で学習は継続します)
./show_training_logs.sh

# 【手動停止】(いつでも手動停止可能・最良モデルは model.json に自動保存)
./stop_parallel_training.sh
```

#### ② フォアグラウンド対話モード（端末で進捗バーを見る場合）
```bash
# 【書式】 ./run_parallel_training.sh <トレーニング回数/ラウンド> <繰り返す回数(0=無制限)>
./run_parallel_training.sh 100 5    # 100回 x 5ラウンド
./run_parallel_training.sh 500 10   # 500回 x 10ラウンド
./run_parallel_training_100.sh 5    # 100回専用 x 5ラウンド
```

> **🛡️ ラウンドごとのメモリ完全解放 & 自動昇格の仕組み**:
> - 各ラウンド（4体 × N回探索）が終了するたびに、4つのワーカープロセスが一度**完全に終了（OSへヒープメモリを100%返還）**します。
> - 最優秀モデルを `model.json` に上書き保存し、次のラウンドで新しいクリーンなプロセスが最新モデルを引き継いで起動します。
> - 指定した「繰り返す回数」に到達すると、全プロセスが自動で終了してメモリを完全解放します。

> **🛡️ SSH切断保護 & 自動継続（永続ループ）の仕組み**:
> - **SIGHUP無効化 & nohup/disown**: SSH接続が突然切断されても、プロセスグループが切り離されているため停止せず学習を続行します。
> - **自動モデル昇格**: 各ラウンド（4体 × N回探索）完了ごとに最優秀モデルを `model.json` に自動マージし、次ラウンドへ最新の重みを引き継いでエンドレスで進化します。
> - **いつでも安全停止**: `./stop_parallel_training.sh` を実行すれば、その時点の最良重みを `model.json` に保存して安全に終了します。

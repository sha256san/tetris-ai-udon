...
### 3. 4体同時 10,000回反復 並列進化学習バッチの起動 & 停止
```bash
# 【開始】4体のワーカーをバックグラウンド並行起動（10,000回ごとに最優秀モデルを model.json へ自動マージし、自動で次ラウンドへ永久継続）
./run_parallel_training_10000.sh
...
> **自動継続（永続ループ）の仕組み**:
> - 各ラウンド（4体 × 10,000回 = 計40,000ゲーム探索）が完了するたびに、4体の中で最も高いFitnessを達成したモデルを自動判定します。

...
#### ① SSH切断完全耐性 デーモンモード（SSHが突然切れても学習を継続）
```bash
# 【デーモン起動 (バックグラウンド永続実行)】
./start_training_daemon.sh 500   # 500回/ラウンド (推奨)
# または ./start_training_daemon.sh 100 (100回/ラウンド)

# 【リアルタイムログ確認】(確認を終了しても学習は裏で動き続けます)
...
```bash
# 端末を開いたまま進捗バーを見る場合
./run_parallel_training.sh 500   # 500回/ラウンド
./run_parallel_training_100.sh   # 100回/ラウンド
```

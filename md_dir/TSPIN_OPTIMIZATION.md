...
## 5. T-Spin 内訳 & 火力 (APM) 詳細分析表 (実戦ベンチマーク検証: 各400ミノ × 5シード平均)

HoikoCode流のTD砲検出・`TrustRate`減衰・`WasteT`防止・`HoldT`温存ロジックを統合した最新の探索アルゴリズムにおける実戦測定値です：

| アルゴリズム構成 | TSS | TSD | TST | Mini | **T-Spin 総計** | **T-Slot 形成** | Tetris | **平均消去ライン** | **平均スコア** | **APM (送信段/分)** |
|---|---|---|---|---|---|---|---|---|---|---|
| **Beam Search (Depth 5, Width 30) [Vulkan wgpu]** | **5.60 回** | **10.80 回** | **0.00 回** | **23.00 回** | **39.40 回** | **189.8 回** | **9.4 回** | **102.2 行** | **47,723 点** | **412.2 APM** |
| **Beam Search (Depth 5, Width 30) [ROCm HIP]** | **5.60 回** | **10.80 回** | **0.00 回** | **23.00 回** | **39.40 回** | **189.8 回** | **9.4 回** | **102.2 行** | **47,723 点** | **311.5 APM** |
| **Beam Search (Depth 3, Width 50) [Vulkan wgpu]** | **8.00 回** | **7.80 回** | **0.00 回** | **22.60 回** | **38.40 回** | **236.4 回** | **8.2 回** | **97.0 行** | **43,196 点** | **353.1 APM** |
| **Beam Search (Depth 3, Width 50) [ROCm HIP]** | **8.00 回** | **7.80 回** | **0.00 回** | **22.60 回** | **38.40 回** | **236.4 回** | **8.2 回** | **97.0 行** | **43,196 点** | **266.1 APM** |
| **Beam Search (Depth 3, Width 50) [CPU Multi-thread]** | **0.60 回** | **2.00 回** | **0.00 回** | **9.40 回** | **12.00 回** | **138.0 回** | **1.4 回** | **17.4 行** | **9,719 点** | **590.8 APM** |
| **Base 1-Ply (No Lookahead) [ROCm HIP]** | **3.00 回** | **1.00 回** | **0.00 回** | **8.40 回** | **12.40 回** | **109.2 回** | **0.2 回** | **16.8 行** | **7,428 点** | **13,357.2 APM** |

### 5.1 改善ポイント
1. **TSD（T-Spin Double）発火数が 5.00回 $\rightarrow$ 10.80回 へ倍増**:
   - `WasteT` 抑制と `HoldT` 温存シナジーにより、Tミノが無駄消費されず確実にTSD発火へと誘導。
2. **平均消去ライン数が 61.2行 $\rightarrow$ 102.2行 へ約1.7倍向上**:
   - 地形崩壊率が大幅に低下し、400ミノ完走に近い高安定性を獲得。
3. **平均スコアが 24,958点 $\rightarrow$ 47,723点 へ約1.9倍向上**:
   - 連続B2B維持とTetris(9.4回)・TSD(10.8回)の複合連打による高得点化を達成。
